...
template <typename T>
double vector_mean(std::vector<T>& v) {
    return std::accumulate(v.begin(), v.end(), 0.0) / v.size();
}


inline void union_sets(std::unordered_set<int>& source_set, std::unordered_set<int>& target_set) {
    for (auto source : source_set) target_set.insert(source);
}

std::vector<int> blocked_nodes_values(double geom_coef, int n) {
    std::vector<int> samples({0});
    samples.reserve(n);
    double v = geom_coef;
    while (samples.back() + (int)v < n) {
        samples.push_back(samples.back() + (int)v);
        v *= geom_coef;
    }
    return samples;
}
