...
dagger::DagWrapper::DagWrapper(Graph& org, ReachabilityLabels& _labels_to_maintain):orig(org),labels_to_maintain(_labels_to_maintain){
	nextNumber = -1;
	dagsize = 0;
	constructDag();
}

//dagger::DagWrapper::DagWrapper(){}

dagger::DagWrapper::~DagWrapper(){
}

void dagger::DagWrapper::printStatistics(){
	int max = 1;
...
void dagger::DagWrapper::tarjan(int node){
...
void dagger::DagWrapper::constructDag(){
...
int dagger::DagWrapper::dagSize(){
	return dagsize;
}

int dagger::DagWrapper::getNextId(){
	return nextNumber--;
}

int dagger::DagWrapper::getNodeSize(int index){
	return index >= 0 ? 1 : csize[index];
}


int dagger::DagWrapper::addDagEdge(int src, int trg, int inc=1){
	int res = 0;
	if(src==trg)
		return 0;
...
int dagger::DagWrapper::removeDagEdge(int src, int trg){
	int res = 0;
...
int dagger::DagWrapper::getScc(int index){
...
int dagger::DagWrapper::getSccOld(int index){
	int cur=index, lsize = 0,nextVal,tmp;
	MyMap::iterator next = sccmap.find(index);
...
EdgeList& dagger::DagWrapper::getOutEdgesFast(int index){
	return (index >= 0)? orig[index].outEdges() : vertices[index].outEdges();
}

EdgeList& dagger::DagWrapper::getInEdgesFast(int index){
	return (index >= 0)? orig[index].inEdges() : vertices[index].inEdges();
}

EdgeList& dagger::DagWrapper::getOutEdges(int index, EdgeList& trick){
	if(index >= 0){
		trick.clear();
		EdgeList::iterator eit;
...
EdgeList& dagger::DagWrapper::getInEdges(int index, EdgeList& trick){
	if(index >= 0){
		trick.clear();
		EdgeList::iterator eit;
...
void dagger::DagWrapper::writeMap(){
	MyMap::iterator mit;
...
Vertex& dagger::DagWrapper::operator [](int index){
	if(index >=0 )
		return orig[index];
	else
...
Vertex& dagger::DagWrapper::getVertex(int index){
	if(index >=0 )
		return orig[index];
	else
		return vertices[index];
...
void dagger::DagWrapper::insertEdge(int u, int v, dagger::Grail*gr){
	// maintain labels at labels_to_maintain
...
void dagger::DagWrapper::updateDagAfterMerge(int& center, vector<int>& list){
	int mapto;
	...
	for(int i=0;i<list.size();i++){
		if(list[i]!=center){
			labels_to_maintain.unset(list[i]); // invalidate index when propagating indexes of reachability
			if (list[i] > 0) {
			    single_nodes.erase(list[i]);
			}
			if (roots.count(list[i])) {
			    roots.erase(list[i]);
			}
...
void dagger::DagWrapper::removeNode(int u){
...
int dagger::DagWrapper::mergeComponents(int src, int trg, vector<int>& list, int & center, dagger::Grail* gr){
	visited[src] = opCnt;
	EdgeList _trick;
...
void dagger::DagWrapper::deleteEdge(int u, int v, dagger::Grail*gr){
...
void dagger::DagWrapper::updateDagEdgesAfterSplit(int& oldcenter, MySet& clist){
	MyMap::iterator mit;
	EdgeList::iterator eit;
...
int dagger::DagWrapper::extractComponents(int node, int trg, int dagnode, MySet& clist){
	int tmp1,tmp2;
	if(node == trg){
...
void dagger::DagWrapper::formComponentDuringSplit(int node, int dagnode,MySet& clist){
