...
dagger::Grail::Grail(Graph& graph, int Dim, int labelingType, int increment, ReachabilityLabels& _labels_to_maintain): size(graph.size()),
		dim(Dim),dag(graph, _labels_to_maintain), inc(increment) {
...
void dagger::Grail::integrityCheck(){
	LabelList::iterator lit;
...
int dagger::Grail::query(int src, int trg){
	if(src>=0)
		src = dag.getScc(src);
	if(trg>=0)
		trg = dag.getScc(trg);
...
void dagger::Grail::edgeAdded(int src, int trg){
	dag.insertEdge(src,trg,this);
}

void dagger::Grail::edgeDeleted(int src, int trg){
	dag.deleteEdge(src,trg,this);
}

void dagger::Grail::nodeDeleted(int nid){
	labels.erase(nid);
	dag.removeNode(nid);
}
void dagger::Grail::nodeAdded(int src, vector<int>& incoming, vector<int>& outgoing){
	dag.single_nodes.insert(src);
	dag.roots.insert(src);
...
void dagger::Grail::componentsMerged(int center, vector<int>& list){
...
void dagger::Grail::componentSplit(int center, MySet& clist, int trg){
	MySet::iterator mit;
//	dag.writeMap();
//	cout << "CS 1" << endl;
...
void dagger::Grail::splitVisit(int s, MySet& clist, int stval, int endval, int d, int& counter, PQueue& pqueue){
	int end = counter;
	EdgeList _trick;
	vector<int> shuffled;
...
void dagger::Grail::dagEdgeAdded(int parent, int child){
...
void dagger::Grail::dagEdgeRemoved(int parent, int child){
//	cout << "removing dag edge" << endl;
	// do nothing
}
void dagger::Grail::propagateUpStart(int parent, int d, int val){
	EdgeList temp;
...
void dagger::Grail::propagateUpEnd(int parent, int d, int val){
	PQueue pqueue;
...
void dagger::Grail::pQueueProcess(PQueue& pqueue, int d){
	PQueue::iterator eit;
	int parent;
...
//@todo sometimes it is stuck here
	}	
}


int dagger::Grail::rquery(int src, int trg){
	visited[src] = _opCnt;
//	dag.writeGraph(cout);
	if(src == trg)
		return 1;
...
int dagger::Grail::contains(int src, int trg){
	label& sl=labels[src];
	label& tl=labels[trg];
	for(int i=0;i<dim;i++){
...
bool dagger::Grail::containsNode(int src) {
	return labels.count(src) == 1;
}

void dagger::Grail::randomLabeling(int tra){
...
void dagger::Grail::visit(int node, int tra, int& post){
	visited[node]=_opCnt;
	vector<int>::iterator sit;
	vector<int> shuffled;
	EdgeList _trick;
...
void dagger::Grail::extractList(EdgeList::iterator first, EdgeList::iterator last, std::back_insert_iterator<vector<int> > result )
{
	while (first!=last){
		*result = first->first;
		first++;
...
void dagger::Grail::printStatistics(){
	dag.printStatistics();
}

void dagger::Grail::reverseLabeling(int tra){
	
}
void dagger::Grail::debug(int nid){
	EdgeList::iterator eit;
